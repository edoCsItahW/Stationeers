...
function Invoke-CMakeConfigure {
    param([string]$BuildDir = "build", [string]$SourceDir = ".", [string]$CoreArtifactDir = "", [string[]]$ExtraArgs = @())
...
function Update-CoreArtifactCache {
    ...
    $ic10Lib = Join-Path $BuildDir "core" $Config "ic10_lib.lib"
    ...
    if (Test-Path $ic10Lib) {
        Copy-Item -Path $ic10Lib -Destination (Join-Path $cacheLibDir "core" $Config) -Force
        Write-Info "已缓存 ic10_lib.lib"
    }

    Copy-Item -Path (Join-Path $PWD ".." ".." ".." "common" "cpp" "core" "include" "*") -Destination (Join-Path $cacheIncludeDir "common") -Recurse -Force
    Copy-Item -Path (Join-Path $PWD "core" "include" "*") -Destination (Join-Path $cacheIncludeDir "IC10") -Recurse -Force
    Write-Info "已缓存头文件"
