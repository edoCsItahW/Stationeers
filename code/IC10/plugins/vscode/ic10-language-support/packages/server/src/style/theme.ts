 ...
 * */
import { EventEmitter } from "@ic10/common";
 ...
 * */
// 快捷调用（自动使用当前主题）
export const s: <P extends StylePaths<ThemeSchemas[ThemeName]>>(
    path: P
) => ValueAtPath<ThemeSchemas[ThemeName], P> = themeController.getCurrent.bind(themeController);
 ...
 * */
// 也可以直接导出全路径方法
export const getStyle: <P extends AllPaths>(path: P) => ValueAtPath<ThemeSchemas, P> =
    themeController.get.bind(themeController);
