...
9. 使用esbuild打包为单文件
   ```cmd
   pnpm exec esbuild ./out/server/server.js --bundle --platform=node --outfile=bundle.js --external:ic10c_node 
   ```
...
2. 在原第10步位置改为手动编辑bundle.js（位于server目录位置）
   ...
   - 在文件开头，"use strict"之后添加
      ...
      const tmpFile = path.join(os.tmpdir(), "ic10c_node.node");
      fs.writeFileSync(tmpFile, new Uint8Array(getRawAsset("ic10c_node.node")));
      const api = { exports: {} };
      process.dlopen(api, tmpFile);
         
      global.ic10NodeApi = api.exports;
      ```
   - 将*bundle.js*中所有`require("ic10c_node")`的地方，替换为`global.ic10NodeApi`
   - 将*bundle.js*中所有`require("ic10c_node/static/stdLib.ic.json")`替换为`JSON.parse(require('node:sea').getAsset('stdLib.ic.json', 'utf8'))`
...
2. **At the original step 10, instead, manually edit `bundle.js`**
   ...
   - At the beginning of the file, after `"use strict"`, add:
     ...
     global.ic10NodeApi = api.exports;
     ```
   - Replace **all occurrences** of `require("ic10c_node")` in `bundle.js` with `global.ic10NodeApi`.
   - Replace **all occurrences** of `require("ic10c_node/static/stdLib.ic.json")` with `JSON.parse(require('node:sea').getAsset('stdLib.ic.json', 'utf8'))`.
