 ...
 * */
import { ASTNode, Program, OperandNode } from "ic10-node-api";
...
export function visit<T extends NodeType = NodeType, R = any>(visitor: { [K in T["type"]]: (node: ExtractNodeByType<K>) => R }, node: T): R {
    return visitor[node.type as T["type"]](node as any);
}

export function visitOperand<R = any>(visitor: { [K in OperandNode["type"]]: (node: Extract<OperandNode, { type: K }>) => R }, node: OperandNode): R {
    return visitor[node.type as OperandNode["type"]](node as any);
}

export type HandleGroup<T extends NodeType["type"] = NodeType["type"], R = any> = { [K in T]: (node: ExtractNodeByType<K>) => R; };

export function groupHandlers<T extends NodeType["type"], R = any>(types: T[], handler: (node: ExtractNodeByType<T>) => R): HandleGroup<T, R> {
...
export function operandValueLength(node: OperandNode): number {
    return visitOperand({
        ...groupHandlers(["Device", "Register", "HexNumber", "BinaryNumber", "Identifier"], node => node.value.length),
