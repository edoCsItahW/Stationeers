<#
.SYNOPSIS
    从 pnpm 迁移到 npm：删除所有 pnpm-lock.yaml 和 node_modules，并在每个 package.json 所在目录运行 npm i。
.DESCRIPTION
    该脚本会递归遍历指定根目录（默认为当前目录），删除所有 pnpm-lock.yaml 文件和 node_modules 文件夹，
    然后查找所有包含 package.json 的目录，并依次执行 npm i 安装依赖。
.PARAMETER Root
    项目根目录路径，默认为当前目录。
.PARAMETER Force
    强制执行，跳过确认提示。
.EXAMPLE
    .\Convert-PnpmToNpm.ps1
    在当前目录执行迁移。
.EXAMPLE
    .\Convert-PnpmToNpm.ps1 -Root "C:\MyProject" -Force
    在指定路径执行迁移，无需确认。
#>
...
Write-Host "Converting from pnpm to npm in '$Root'" -ForegroundColor Cyan
...
# 确认操作（除非使用 -Force）
if (-not $Force) {
    $confirmation = Read-Host "This will delete ALL pnpm-lock.yaml and node_modules folders recursively. Are you sure? (y/n)"
...
# ---------- 1. 删除所有 pnpm-lock.yaml ----------
Write-Host "`nDeleting pnpm-lock.yaml files..." -ForegroundColor Yellow
$lockFiles = Get-ChildItem -Path $Root -Filter "pnpm-lock.yaml" -Recurse -File
$lockCount = 0
...
Write-Host "Deleted $lockCount pnpm-lock.yaml file(s)." -ForegroundColor Green
...
# ---------- 3. 在每个包含 package.json 的目录中运行 npm i ----------
Write-Host "`nInstalling dependencies with npm in each package..." -ForegroundColor Yellow
...
foreach ($file in $packageJsonFiles) {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Warning "npm i failed in '$($dir.FullName)' (exit code: $LASTEXITCODE)"
        }
    } catch {
