...
> [!TIP]
> 格式化配置也可通过项目根目录下的 `.ic.json`、`.ic.yaml` 或 `.ic.yml` 文件指定，格式参考 [参考模板]() 模板。

## 标准库与类型系统

编译器内置了标准库（[stdLib.ic](https://github.com/edoCsItahW/Stationeers/blob/develop/code/backend/compiler/tests/stdLib.ic)），定义了常见设备的类型、逻辑槽位、批处理模式等枚举。但由于游戏中设备种类繁多，标准库目前仅覆盖了部分设备。

如果你的项目使用了尚未纳入标准库的设备类型，欢迎向标准库仓库贡献。标准库使用特殊的文档注释语法（`#>`）定义枚举和设备类型：
...
1. **IC10 标准兼容性**：IC10 存在多种社区实现标准，编译器以 Stationeers 官方行为为准，但部分边缘语法可能与某些社区标准不完全兼容。诊断结果应以游戏实际运行为最终依据。

2. **标准库覆盖不全**：由于游戏中设备数量庞大，标准库尚未覆盖所有设备类型。未注册设备将回退到全局枚举补全，精度受限。

3. **大文件性能**：虽然增量编译大幅降低了编辑延迟，但在首次打开超大文件时（数千行），全量解析仍需一定时间。
...
7. **Code Formatting**
   Supports configuration from `.editorconfig` or a project-root `ic10-format.yaml`. Formatting rules:
...
> [!TIP]
> Formatting configuration can also be specified via an `ic10-format.yaml` or `.editorconfig` file in the project root. See the `ic10-format.yaml` template for the schema.

## Standard Library & Type System

The compiler ships with a built-in standard library ([stdLib.ic](https://github.com/edoCsItahW/Stationeers/blob/develop/code/backend/compiler/tests/stdLib.ic)) that defines enumerations for common device types, logic slots, batch modes, and more. However, given the vast number of in-game devices, the standard library currently covers only a subset.

If your project uses device types not yet in the standard library, contributions are welcome. The standard library uses a special documentation comment syntax (`#>`) to define enumerations and device types:
...
1. **IC10 Standard Compatibility**: IC10 has multiple community implementation standards. The compiler targets official Stationeers behavior, but some edge-case syntax may not be fully compatible with all community standards. Diagnostic results should be verified against actual in-game behavior.

2. **Standard Library Coverage**: Due to the large number of in-game devices, the standard library does not yet cover all device types. Unregistered devices fall back to global enum completion with reduced precision.

3. **Large File Performance**: While incremental compilation significantly reduces editing latency, the initial full parse for very large files (thousands of lines) still takes a noticeable amount of time.
