...
#include <type_traits>
    ...
    #define WIN32_LEAN_AND_MEAN  // 剔除不常用的 API 声明（如 Winsock 等），加快编译
    #define VC_EXTRALEAN         // 同上，进一步精简
    #define NOMINMAX             // 禁止定义 min 和 max 宏
    #define NOGDI                // 禁止定义 GDI 相关类型（如 HDC、HPEN），若无需绘图可加上
    #define NOSERVICE            // 禁止定义服务相关结构
    #define NOMCX                // 禁止定义 Modem 扩展
